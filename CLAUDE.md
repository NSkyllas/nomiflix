...
**Credits are parsed, fail-loud, and split two ways** — the upload form has an optional Credits field for pasting IMDB's "Full Cast & Crew" page as-is. `scripts/parse_credits.py` parses it into department-grouped records (mirrors the spirit of Nomify's Discogs-credits parser, adapted to IMDB's different and messier format). Director/Writer/Cast — the only three categories Jellyfin's NFO schema has slots for — feed into the NFO; everything else (Producer, Composer, every other department, plus alias/uncredited/voice detail) only lives in a separate `<video basename>-credits.json`, meant for future analysis/visualization, same spirit as Nomify's `credits.json`. An unparseable paste fails the **whole item** (routed to `_Failed/`, matching Nomify's fail-loud philosophy for its own credits/tracklist parsing) rather than degrading gracefully. There's no separate Overview/plot field; Credits replaces that role. See `docs/SPEC.md` §6.1.
