...
- `process_item.py` — done. Orchestrates one `_Processing/<base>.*` pair: probe → transcode (if needed) → move into library → place poster/credits (if any) → write NFO, or route to `_Failed/` on any expected error. Verified end-to-end against synthetic files (transcode branch, skip branch, corrupt-input failure) and against a real Genre+Credits upload
